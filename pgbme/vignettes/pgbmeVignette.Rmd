...
In this document, we provide a brief a tutorial on how to use the P-GBME model detailed in ["Modeling Asymmetric Relationships from Symmetric Networks"](https://arxiv.org/abs/1711.03838). The P-GBME model can be run by installing the package using devtools -- in the future, we may incorporate this model into the [AMEN framework](https://arxiv.org/abs/1611.00460): 

```{r, eval=FALSE}
# first install package using devtools
library(devtools)
install_github(repo='pgbmeRepl', username='s7minhas', subdir='pgbme')
```
...
library(pgbme)
```
The main function is `pgbme` and it relies on five packages: 
