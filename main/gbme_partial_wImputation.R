...
#XdStart : a n*n*rd array representing dyad-specific predictors that will be used to set starting values
#XsStart : a n*rs matrix of sender-specific predictors that will be used to set starting values
#XrStart : a n*rr matrix of receiver-specific predictors that will be used to set starting values
...
gbme.ef_wImp <-function(
  y = y,  
  Xd_L = list(array(dim=c(nrow(Y.start(y)),nrow(Y.start(y)),0))),
  Xs_L = list(matrix(nrow=nrow(Y.start(y)),ncol=0)),
  Xr_L = list(matrix(nrow=nrow(Y.start(y)),ncol=0)), 
  XdStart = array(dim=c(nrow(Y.start(y)),nrow(Y.start(y)),0)),
  XsStart = matrix(nrow=nrow(Y.start(y)),ncol=0),
  XrStart = matrix(nrow=nrow(Y.start(y)),ncol=0),
  ...
  rho.calc = TRUE, # Estimate rho by default
  #Details of output
...
)
{
  ...
  X.u_L = lapply(1:length(Xs_L), function(i){ cbind(rep(.5,2*n), adiag(Xs_L[[i]], Xr_L[[i]]))  })
  rm(list=c('Xs_L','Xr_L'))
  ...
  if (is.null(startv)){
    ...
    startv$beta.u <- c(fixef(mle)[1], rep(0, ncol(X.u_L[[1]])-1))
  ...
  tmp <- lapply(Xd_L, function(x){ XuXv(x) })
  ...
  for(ns in 1:NS){  
    ...
    if(ns > burn & (ns-burn)%%odens==0){  
      ...
      if(write) { 
        if(n.samp==1)   {write.table(t(out),file="theta", quote=F, row.names=F, col.names=cnames)}
        if(n.samp > 1)  { write.table(t(out),file="theta", append=T, quote=F, row.names=F,col.names=F) }
      }
      
      if(!write) {cat(out,"\n") }
      
      if(k > 0) { 
        write.table(t(e), file = "u", append = T*(n.samp > 1), quote=F, row.names=F, col.names=F) 
        write.table(t(f), file = "v", append = T*(n.samp > 1), quote=F, row.names=F, col.names=F)      
      }
      
      write.table(t(a), file="a", append = T*(n.samp>1), quote = F, row.names = F, col.names = F)  
      write.table(t(b), file="b", append = T*(n.samp>1), quote = F, row.names = F, col.names =F)
