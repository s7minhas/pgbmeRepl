...
s <- rep(1:n, n-1)
mle  <- glmer( c(mat.vect(Y)) ~
  Xdyad + Xs[rep(1:n, n-1), ] + (1|s),
  family=binomial(link=probit) )

priors = list()
priors$pi.s2u <- c(1, 1) # inv-gamma: appriori mean approx 1  
priors$pi.s2z <- matrix(c(1, 1), max(k, 1), 2, byrow=TRUE) # inv-gamma for z's
priors$pim.bd <- rep(0, rd) 
priors$piS.bd <- 10*diag(rd)

startv=list()
startv$beta.d <- fixef(mle)[grepl("Xdyad", names(fixef(mle)))]
startv$beta.u <- c(fixef(mle)[1], fixef(mle)[grepl("Xs", names(fixef(mle)))])
startv$z <- matrix(0, n, max(k,1))
startv$s <- ranef(mle)$s[,1]

tmp2<-lm(startv$s~-1+cbind( rep(.5,n),Xs)) 
priors$pi.s2a<-c(2,var(tmp2$res))
priors$pim.b0s <- rep(0, 1 + rs) 
priors$piS.b0s <- 10*diag(1 + rs)   
}
...
for(ns in 1:NS){
if(lpy.th==-Inf){save.image(file='wackyShit.rda') ; stop('ll got wacky')}
